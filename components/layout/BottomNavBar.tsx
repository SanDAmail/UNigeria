...
const BottomNavBar: React.FC = () => {
    ...
    const tabs = [
        ...
        { id: ListPanelTab.FORUMS, icon: Icons.Flag, label: 'Forums' },
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-ui-border flex justify-around items-start pt-2 pb-1 z-30">
            {tabs.map(tab => {
                let isActive = false;
                ...
                return (
                    ...
                    >
                       <tab.icon className={`w-6 h-6 transition-colors ${isActive ? 'text-primary-green' : 'text-secondary'}`} />
                       <span className={`transition-colors text-center ${isActive ? 'text-primary-green font-semibold' : 'text-secondary'}`}>{tab.label}</span>
                    </button>
                )
            })}
        </div>
