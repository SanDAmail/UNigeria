...
const ChatListItem: React.FC<ChatListItemProps> = ({ data, isActive, onClick }) => {
  ...
  if (data.id === 'nigeria') {
    return (
      <div
        onClick={onClick}
        className={`p-4 border-b border-ui-border cursor-pointer transition-colors duration-150 ${
          isActive ? 'bg-primary-green/10' : 'hover:bg-app-light'
        }`}
      >
        <div className="flex flex-col items-center text-center">
          <img src={avatar} alt={name} className="w-16 h-16 rounded-full border-2 border-white shadow-md" />
          <p className="font-bold text-lg text-primary mt-3">{name}</p>
          <p className="text-sm text-secondary italic mt-1">{`"${lastMessage}"`}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      onClick={onClick}
      className={`flex p-3 items-start space-x-3 border-b border-ui-border cursor-pointer transition-colors duration-150 ${
        isActive ? 'bg-primary-green/10' : 'hover:bg-app-light'
      }`}
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm text-secondary truncate">{isState ? `"${lastMessage}"` : lastMessage}</p>
      </div>
      <div className="flex flex-col items-end space-y-1 text-xs text-secondary self-start">
        <span>{timestamp}</span>
        {unreadCount > 0 && (
