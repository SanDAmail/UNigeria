...
try {
  if (process.env.API_KEY) {
    ...
    console.error("API_KEY environment variable not set for townHallService.");
  }
} catch (e) {
  console.error("Failed to initialize GoogleGenAI in townHallService", e);
}
...
class TownHallService {
  ...
  public async generateReply(
    ...
    const replySystemInstruction = `You are an AI moderator managing a Town Hall discussion. The history contains a discussion with multiple viewpoints. The user has just replied. Your task is to generate a SINGLE, relevant response from one of the existing or a new appropriate stakeholder.
    ...
    try {
      ...
      console.error("Error generating/parsing Town Hall reply JSON:", error);
      console.error("Raw AI Response that failed parsing:", rawResponseText);
      throw new Error("Failed to generate a Town Hall reply. Please try again.");
    }
  }
}
